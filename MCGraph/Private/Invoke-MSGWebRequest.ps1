...
function Invoke-MSGWebRequest {
    ...
    process {
        ...
        if ($Force -or $PSCmdlet.ShouldProcess("$uri")) {
            $response = try {
                if ($body) {
                    (Invoke-WebRequest -Method $method -Uri $uri -ContentType $contentType -Headers @{Authorization = "Bearer $($token.value)" } -Body $body -UseBasicParsing -ErrorAction Stop)
                }
                else {
                    (Invoke-WebRequest -Method $method -Uri $uri -ContentType $contentType -Headers @{Authorization = "Bearer $($token.value)" } -UseBasicParsing -ErrorAction Stop)
                }
            }
